...
export const sendCurl = createAsyncThunk<
  ...
  number,
  { state: { activeDocument: ActiveDocumentState } }
>('activeDocument/sendCurl', async (cellIndex, thunkAPI) => {
  const state = thunkAPI.getState().activeDocument as ActiveDocumentState;
  if (!state) {
    throw new Error('No active document');
  }
  ...
  const responses = await Promise.race([
    Services.sendCurl(
      cell.source.join('\n'),
      _([...preScriptVariables, ...state.globalVariables])
        .uniqBy('key')
        .value(),
    ).catch((error) => {
      throw new Error(
    ...
    .then(
      (responses) => responses as Awaited<ReturnType<typeof Services.sendCurl>>,
    )
    .then((responses) =>
...
export const validateCellAndSendCurl = createAsyncThunk<
  ...
  number,
  { state: { activeDocument: ActiveDocumentState } }
>('activeDocument/validateCellAndSendCurl', async (cellIndex, thunkAPI) => {
  const state = thunkAPI.getState().activeDocument as ActiveDocumentState;
  if (!state) {
    throw new Error('No active document');
  }
  ...
  const { responses } = await thunkAPI.dispatch(sendCurl(cellIndex)).unwrap();
  return {
    responses,
  };
});
...
export const sendAllCurls = createAsyncThunk<
  ...
  void,
  { state: { activeDocument: ActiveDocumentState } }
>('activeDocument/sendAllCurls', async (__, thunkAPI) => {
  const state = thunkAPI.getState().activeDocument as ActiveDocumentState;
  if (!state) {
    throw new Error('No active document');
  }
  ...
  await Bluebird.mapSeries(state.cells, async (cell, cellIndex) => {
    ...
    if (
      ...
      await thunkAPI.dispatch(sendCurl(cellIndex));
    }
  });
});
...
export const activeDocumentSlice = createSlice({
  ...
  extraReducers: (builder) => {
    const sendCurlPending = (
      ...
      state.activeCellIndex = action.meta.arg;
      const cell = state.cells[action.meta.arg];
